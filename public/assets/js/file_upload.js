...
function readLoginURL(input) {
    if (input.files && input.files[0]) {
        var reader = new FileReader();
        reader.onload = function(e) {
            $('#imagePreviewLogin').css('background-image', 'url('+e.target.result +')');
            $('#imagePreviewLogin').hide();
            $('#imagePreviewLogin').fadeIn(650);
        }
        reader.readAsDataURL(input.files[0]);
    }
}
function readSignupURL(input) {
    if (input.files && input.files[0]) {
        var reader = new FileReader();
        reader.onload = function(e) {
            $('#imagePreviewSignup').css('background-image', 'url('+e.target.result +')');
            $('#imagePreviewSignup').hide();
            $('#imagePreviewSignup').fadeIn(650);
        }
        reader.readAsDataURL(input.files[0]);
    }
}
...
$("#imageUploadLogin").change(function() {
    readLoginURL(this);
});
$("#imageUploadSignup").change(function() {
    readSignupURL(this);
});
